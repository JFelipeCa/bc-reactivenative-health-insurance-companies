...
import { FlatList, KeyboardAvoidingView, Pressable, RefreshControl, SafeAreaView, ScrollView, StyleSheet, Text, TextInput, View, Image } from 'react-native';
import { fetchCoverages } from '../api/coverages';
import { useHealthStore } from '../store';

type Coberturas = { id: string; name: string; category: string; detail: string };
type RootStackParamList = { MainTabs: undefined; CoberturasDetail: { coverage: Coberturas }; Enrollment: undefined };
  ...
  return (
    <SafeAreaView style={styles.safeArea}>
      <ScrollView contentContainerStyle={styles.homeContent} showsVerticalScrollIndicator={false}>
        ...
        <Text style={styles.sectionTitle}>{String.fromCodePoint(0x2728)}  Accesos rápidos</Text>
        ...
        <View style={styles.infoCard}><Text style={styles.infoKicker}>PLAN ACTIVO</Text><Text style={styles.infoTitle}>{selectedPlan}</Text><Text style={styles.infoText}>protección para tu hogar con copagos y red nacional. Tu preferencia se guarda localmente.</Text></View>
      </ScrollView>
    </SafeAreaView>
  );
}
...
export default function App() {
  return <QueryClientProvider client={queryClient}><NavigationContainer><StatusBar style="dark" /><Stack.Navigator><Stack.Screen name="MainTabs" component={MainTabs} options={{ headerShown: false }} /><Stack.Screen name="CoberturasDetail" component={DetailScreen} options={{ title: 'Detalle de cobertura' }} /><Stack.Screen name="Enrollment" component={EnrollmentScreen} options={{ title: 'Afiliación' }} /></Stack.Navigator></NavigationContainer></QueryClientProvider>;
}
...
const styles = StyleSheet.create({
  ...
  hero: { backgroundColor: '#DFF3E8', borderRadius: 22, alignItems: 'center', flexDirection: 'row', gap: 12, overflow: 'hidden', padding: 22, marginTop: 10 },
